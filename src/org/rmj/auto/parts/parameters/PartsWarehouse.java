...
public class PartsWarehouse {
    private final String MASTER_TABLE = "warehouse";
    ...
    public PartsWarehouse(GRider foGRider, String fsBranchCd, boolean fbWithParent){            
        poGRider = foGRider;
...
//    }
    ...
    public boolean searchRecord() throws SQLException{
        ...
        if (loJSON == null){
            ...
            if (OpenRecord((String) loJSON.get("sWHouseID")) ){
            }else {
                psMessage = "No record found/selected.";
                return false;
            }
    ...
    public boolean OpenRecord(String fsValue){
        try {
            String lsSQL = "";
            lsSQL = MiscUtil.addCondition(getSQ_Master(), "sWHouseID = " + SQLUtil.toSQL(fsValue));
            ResultSet loRS = poGRider.executeQuery(lsSQL);
    ...
    /**
    Updates the record status of a warehouse.
    @param fsValue The value to identify the warehouse record.
    ...
    */
    public boolean UpdateRecordStatus(String fsValue, boolean fbValue){
        ...
        try {
            ...
            lsSQL = MiscUtil.rowset2SQL(poMaster, 
                                        MASTER_TABLE, 
                                        "", 
                                        "sWHouseID = " + SQLUtil.toSQL(fsValue));
            if (lsSQL.isEmpty()){
        ...
        if (fbValue){
            psMessage = "Warehouse Successfully Activated.";
        } else {
            psMessage = "Warehouse Successfully Deactivated.";
        }
    ...
    public boolean SaveRecord(){
        ...
        try {
            ...
            if (pnEditMode == EditMode.ADDNEW){ //add
                System.out.println(MiscUtil.getNextCode(MASTER_TABLE, "sWHouseID", false, poGRider.getConnection(), psBranchCd) );
                //return true;
                poMaster.updateString("sWHouseID",MiscUtil.getNextCode(MASTER_TABLE, "sWHouseID", false, poGRider.getConnection(), psBranchCd) );                                                             
                poMaster.updateString("sModified", poGRider.getUserID());
                poMaster.updateObject("dModified", (Date) poGRider.getServerDate());
                poMaster.updateRow();
                ...
                lsSQL = MiscUtil.rowset2SQL(poMaster, 
                                            MASTER_TABLE, 
                                            "", 
                                            "sWHouseID = " + SQLUtil.toSQL((String) getMaster("sWHouseID")));
            }
    ...
    private boolean isEntryOK() throws SQLException{
        ...
        if (poMaster.getString("sWHouseNm").isEmpty()){
            psMessage = "Warehouse Name is not set.";
            return false;
        }
        
        String lsSQL = getSQ_Master();
        ResultSet loRS;
        lsSQL = MiscUtil.addCondition(lsSQL, "sWHouseNm = " + SQLUtil.toSQL(poMaster.getString("sWHouseNm")) +
                                                " AND sWHouseID <> " + SQLUtil.toSQL(poMaster.getString("sWHouseID"))); 
        loRS = poGRider.executeQuery(lsSQL);
        if (MiscUtil.RecordCount(loRS) > 0){
            psMessage = "Existing Warehouse Description.";
            MiscUtil.close(loRS);        
            return false;
        }
...
//        loRS = poGRider.executeQuery(lsSQL);
//        if (MiscUtil.RecordCount(loRS) > 0){
//            psMessage = "Warehouse is already used in Item Information. Please contact system administrator to address this issue.";
//            MiscUtil.close(loRS);        
//            return false;
//        }
