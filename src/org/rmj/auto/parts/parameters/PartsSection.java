...
public class PartsSection {
    private final String MASTER_TABLE = "section";
    ...
    public PartsSection(GRider foGRider, String fsBranchCd, boolean fbWithParent){            
        poGRider = foGRider;
...
//    }
    ...
    public boolean searchRecord() throws SQLException{
        ...
        if (loJSON == null){
            ...
            if (OpenRecord((String) loJSON.get("sSectnIDx")) ){
            }else {
                psMessage = "No record found/selected.";
                return false;
            }
    ...
    public boolean OpenRecord(String fsValue){
        try {
            String lsSQL = "";
            lsSQL = MiscUtil.addCondition(getSQ_Master(), "sSectnIDx = " + SQLUtil.toSQL(fsValue));
            ResultSet loRS = poGRider.executeQuery(lsSQL);
    ...
    /**
    Updates the record status of a section.
    @param fsValue The value to identify the section record.
    ...
    */
    public boolean UpdateRecordStatus(String fsValue, boolean fbValue){
        ...
        try {
            ...
            lsSQL = MiscUtil.rowset2SQL(poMaster, 
                                        MASTER_TABLE, 
                                        "", 
                                        "sSectnIDx = " + SQLUtil.toSQL(fsValue));
            if (lsSQL.isEmpty()){
        ...
        if (fbValue){
            psMessage = "Section Successfully Activated.";
        } else {
            psMessage = "Section Successfully Deactivated.";
        }
        ...
        try {
            ...
            if (pnEditMode == EditMode.ADDNEW){ //add
                ...
                lsSQL = MiscUtil.rowset2SQL(poMaster, 
                                            MASTER_TABLE, 
                                            "", 
                                            "sSectnIDx = " + SQLUtil.toSQL((String) getMaster("sSectnIDx")));
            }
    ...
    private boolean isEntryOK() throws SQLException{
        ...
        if (poMaster.getString("sSectnNme").isEmpty()){
            psMessage = "Section Name is not set.";
            return false;
        }
        
        String lsSQL = getSQ_Master();
        ResultSet loRS;
        lsSQL = MiscUtil.addCondition(lsSQL, "sSectnNme = " + SQLUtil.toSQL(poMaster.getString("sSectnNme")) +
                                                " AND sSectnIDx <> " + SQLUtil.toSQL(poMaster.getString("sSectnIDx"))); 
        loRS = poGRider.executeQuery(lsSQL);
        if (MiscUtil.RecordCount(loRS) > 0){
            psMessage = "Existing Section Description.";
            MiscUtil.close(loRS);        
            return false;
        }
